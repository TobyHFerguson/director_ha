...
sudo yum -y install ntp curl nscd screen python bc rng-tools gdisk yum-utils krb5-workstation openldap-clients openssl wget realmd sssd oddjob oddjob-mkhomedir adcli samba-common samba-common-tools 
...
sudo yum -y erase cloudera-manager-agent cloudera-manager-daemons cloudera-manager-server cloudera-manager-server-db-2
